...
private:
	...
	int maxLiftEncDist = UNINIT_VAL; //encoder distance at the top
	int pidPosSetPoint = UNINIT_VAL; //set point for the PID controller
	...
	void SetLiftControl(int val)
	{
		controlLift->SetSetpoint(val);
	}
	...
	int DistToSetpoint()
	{
		if(!(controlLift->IsEnabled()))
			...
			return (liftEncoder->Get() - pidPosSetPoint);
	}
	...
	bool AtSetpoint()
	{
		if(!(controlLift->IsEnabled()))
			...
			return (abs(DistToSetpoint()) < PID_POS_TOL);
	}
	...
	void TeleopInit()
	{
		...
		sprintf(myString, "end teleopinit");
	...
	void TeleopPeriodic()
	{
			...
			switch (liftState)
			{
				case raising:
					...
					if (GetLiftLimitSwitchMax())
					{
						...
						{
							maxLiftEncDist = liftEncoder->Get();
		...
		sprintf(myString, "maxLiftEncDist: %d\n", maxLiftEncDist);
		SmartDashboard::PutString("DB/String 5", myString);
		sprintf(myString, "encoder distance: %d\n", liftEncoder->Get());
